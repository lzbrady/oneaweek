...
class Classes extends Component {
  constructor() {
    ...
    this.moveToShowActs = this.moveToShowActs.bind(this);
  }
  ...
  loadClasses(schoolId) {
    getClasses(schoolId).then(snapshot => {
      ...
      if (snapshot.size > 0) {
        this.setState({
          classes: snapshot.docs.map(c => {
            return {
              id: c.id,
              teacher: c.data().teacher
            };
          })
        });
  ...
  moveToShowActs(clazz) {
    this.setState({
      showActs: true,
      classId: clazz.id,
      classTeacher: clazz.teacher,
      showClasses: false
    });
  ...
  render() {
    return (
      <div>
        {!this.state.showActs && (
          <h1 className="acts-header">
            {this.props.schoolName}({this.props.state})
          </h1>
        )}
        {!this.state.showActs && (
          <p onClick={this.moveBack} className="back-btn">
            &lt; Back to Schools
          </p>
        )}
        {!this.state.showActs &&
          this.state.classes.map((clazz, index) => {
            return (
              <div
                ...
                key={index}
                onClick={() => this.moveToShowActs(clazz)}
              >
                {clazz.teacher}
              </div>
            );
